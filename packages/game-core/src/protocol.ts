...
import type { GameState, Command, Trump } from "./protocol";
import { fullDeck, shuffle, suitOf, rankIndex, type Card, type Suit } from "./cards";
...
function everyoneHasBid(state: GameState): boolean {
  return state.players.every(p => typeof state.bids[p.id] === "number");
}
...
function pickTrickWinner(plays: { playerId: string; card: Card }[], trump: Trump): string {
  // If any trump cards were played (and trump != NT), highest trump wins.
  if (trump !== "NT") {
...
export function step(state: GameState, cmd: Command): GameState {
  switch (cmd.type) {
    ...
    case "TICK": {
      ...
      return autoPlay(state); // timer expired → auto-play for current player
    }

    case "NEXT_TURN": {
      // Only meaningful in Trick; in Bidding we force strict order with PLACE_BID
      return rotateTurn(state);
    }
